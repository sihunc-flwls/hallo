...
def set_memory_efficient_attention(cfg, unet_list):
...
def train_stage2_process(cfg: argparse.Namespace) -> None:
    ...
    lora_temp_save_dir = f"{cfg.output_dir}/lora_temp"

    if accelerator.is_main_process:
        init_output_dir([save_dir, checkpoint_dir, module_dir, validation_dir, lora_temp_save_dir])
    ...
    set_memory_efficient_attention(cfg, [reference_unet, denoising_unet])
    ...
    logger.info("** hallo + MotionDirector **")
        ...
        for _, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                pixel_values_face_mask = get_attention_mask(
                    batch["pixel_values_face_mask"], weight_dtype
                )
                pixel_values_lip_mask = get_attention_mask(
                    batch["pixel_values_lip_mask"], weight_dtype
                )
                pixel_values_full_mask = get_attention_mask(
                    batch["pixel_values_full_mask"], weight_dtype
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", type=str, default="./configs/train/exp/stage2.yaml"
    )
