...
from emo.utils.config import filter_non_none
from emo.utils.util import tensor_to_video
...
class Net(nn.Module):
    ...
    Args:
        ...
        image_enc (nn.Module): CLIPVisionModelWithProjection model used to project the source image onto the face.
        audioproj (nn.Module): The AudioProjector model used to project the audio embeddings onto the face.
        speed_enc (nn.Module): The SpeedEncoder model used to project the audio embeddings onto the face.
    """
    def __init__(
        self,
...
def inference_process(args: argparse.Namespace):
    ...
    pipeline = FaceAnimatePipeline(
        ...
        image_encoder=net.image_enc,
        scheduler=val_noise_scheduler,
        speed_encoder=net.speed_enc,
    )
    ...
    #source_image_clip_img = source_image_clip_img.unsqueeze(0)
    ...
    for t in range(times):
        ...
        audio_tensor = audio_tensor.to(
            device=net.audioproj.device, 
            dtype=net.audioproj.dtype
        )
        audio_tensor = net.audioproj(audio_tensor)
        
        speed_emb = torch.ones(audio_tensor.shape[0]).to(
            dtype=net.speed_enc.dtype, device=net.speed_enc.device
        )
        ...
        #pipeline_output = pipeline(ref_image=pixel_values_ref_img, audio_tensor=audio_tensor, face_emb=source_image_face_emb, face_mask=source_image_face_region, clip_img=source_image_clip_img,speed_emb=head_speed, width=img_size[0],height=img_size[1],video_length=clip_length,num_inference_steps=config.inference_steps,guidance_scale=config.cfg_scale,generator=generator,motion_scale=motion_scale,)
