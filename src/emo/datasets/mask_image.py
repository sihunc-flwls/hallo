...
class EMODataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.use_clip:
            ...
            # make data extractor!
            try:
                image_emb_path = video_meta["image_emb"]
            except:
                # import PIL.Image as Image
                # imgc = Image.open('/home/sihun.cha/Work/toydata/images/AdamKinzinger0_0/0001.png')
                # clip_img = self.clip_image_processor(imgc, return_tensors='pt').pixel_values
                # torch.save(clip_img,'/home/sihun.cha/Work/toydata/image_emb/AdamKinzinger0_0.pt')
                raise FileNotFoundError('needs to process clip imaeg and add to json file')
            
            if not os.path.exists(image_emb_path):
                clip_img = self.clip_image_processor(ref_img_pil, return_tensors='pt').pixel_values
            else:
                clip_img = torch.load(image_emb_path)
...
if __name__ == "__main__":
    data = EMODataset(img_size=(512, 512))
